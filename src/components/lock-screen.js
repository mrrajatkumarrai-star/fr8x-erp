...
export class LockScreen {
  constructor(onUnlocked) {
    ...
    if (urlParams.get("portal") === "customer" || urlParams.get("login") === "customer") {
  ...
  renderCardContent(container, initials, logoUrl, companyName) {
    const isStaff = this.activeTab === "staff";
    ...
    container.innerHTML = `
      ...
      ">
        ...
        <!-- Portal Tabs Toggle -->
        <div style="display: flex; background: rgba(15, 23, 42, 0.6); padding: 4px; border-radius: var(--radius-md); margin-bottom: 24px; border: 1px solid rgba(255,255,255,0.05);">
          <button id="tab-staff-btn" style="
            flex: 1; padding: 8px; border: none; font-size: 12px; font-weight: 600; border-radius: var(--radius-sm); cursor: pointer; transition: var(--transition);
            background: ${isStaff ? 'var(--primary)' : 'transparent'};
            color: ${isStaff ? '#fff' : 'rgba(255,255,255,0.6)'};
          ">Staff Terminal</button>
          <button id="tab-cust-btn" style="
            flex: 1; padding: 8px; border: none; font-size: 12px; font-weight: 600; border-radius: var(--radius-sm); cursor: pointer; transition: var(--transition);
            background: ${!isStaff ? 'var(--primary)' : 'transparent'};
            color: ${!isStaff ? '#fff' : 'rgba(255,255,255,0.6)'};
        ...
        ${isStaff ? this.getStaffHTML() : this.getCustomerHTML(initials, logoUrl, companyName)}
      </div>
    `;
